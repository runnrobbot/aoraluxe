...
const CartDrawer = () => {
  ...
  // Lock body scroll + touch when drawer is open (prevents tap-through on mobile)
  useEffect(() => {
    if (drawerOpen) {
  ...
  return (
    <AnimatePresence>
      {drawerOpen && (
        <>
          {/* Backdrop – z-[60] ensures it truly covers the whole page */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          {/* Drawer – z-[61] sits on top of backdrop */}
          <motion.aside
            role="dialog"
            aria-modal="true"
