...
const ProductModal = ({ product, onClose }: ProductModalProps) => {
  ...
  const { name, category, price, publicId, description, stock, images } = product;
  ...
  const allImages = (() => {
    ...
    const src = publicId ? getOptimizedUrl(publicId, { width: 900 }) : product.imageUrl;
    const thumb = publicId ? getOptimizedUrl(publicId, { width: 160 }) : product.imageUrl;
    return src ? [{ src, thumb }] : [];
  })();

  const activeSrc = allImages[activeIdx]?.src;

  const waText = `Halo AORA LUXE! Saya ingin memesan:\n\n• ${name} (${category || 'Produk'}) — 1x @ Rp ${price?.toLocaleString('id-ID')}\n\nMohon konfirmasinya, terima kasih`;
            ...
            <div className="aspect-[4/3] md:aspect-square relative overflow-hidden">
              ...
              {allImages.length > 1 && (
                <>
                  ...
                  <button onClick={() => setActiveIdx((i) => Math.min(i + 1, allImages.length - 1))} disabled={activeIdx === allImages.length - 1}
              ...
              <div className="flex gap-1.5 p-2 overflow-x-auto">
                {allImages.map((img, i) => (
                  <button key={i} onClick={() => setActiveIdx(i)}
                    ...
                    style={i === activeIdx ? { outline: '2px solid #c9a84c', outlineOffset: '1px' } : {}}
